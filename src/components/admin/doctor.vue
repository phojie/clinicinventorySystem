<template>
  ...
  <v-layout row wrap >
    <v-flex sm12>
      <v-card>
      <v-card-title>
        <div class="font-weight-black">Doctor's List</div>
        <v-spacer></v-spacer>
        <v-text-field
          v-model="search"
          append-icon="search"
      ...
      >
        <template   slot="items" slot-scope="props">
          ...
          <td class="">{{ props.item.fn }} {{ props.item.ln }}</td>
          <td class="">{{ props.item.email }}</td>
          <td class="">{{ props.item.cnumber }}</td>
          <td class=""> <v-chip small color="green lighten-5" text-color="green" v-for="dt in props.item.doctortype" :key="dt.id"> {{dt}}</v-chip> </td>
   ...
   <!-- dialog doctor -->
    ...
    <v-dialog v-model="dialogDoctor" persistent max-width="600px">
      ...
      <v-card>
        <v-card-title>
          <span class="headline">Update Doctor Profile</span>
    ...
    >
      <v-card>
        <v-card-title>
          <span class="body-2 mb-2"> Are you sure you want to delete <span class="error--text"> {{doctorsDetailsProfile.fn}}  {{doctorsDetailsProfile.ln}}</span> as  a Doctor?  </span>
  ...
  methods: {
    deleteNow() {
      ...
      vm.$notify({
        group: 'bottomright',
        type: 'error',
        title: 'Successfully deleted a doctor',
